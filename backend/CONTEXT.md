...
3. Loads `AppUser`.
4. Rejects role `"user"` with `"Only Super Users are Allowed to use this feature."`.
5. Rejects users with `tokens == 0`.
6. Downloads the user's `base_image_path`.
7. Downloads every selected product image URL.
8. Creates `ImageGenerationTask(status="pending")`.
9. Marks task `processing`.
10. Calls `generate_ai_product_image(info_prompt + custom_prompt, base_image, input_images)`.
11. Uploads generated bytes to Supabase Storage at `/generations/<task_id>.jpg`.
12. Creates `ImageGeneration`.
13. Marks task `completed`.
14. Returns serialized `ImageGeneration`.

Known behavior to preserve or fix deliberately:

- The role gate blocks normal `"user"` accounts, so the later token decrement for `role == "user"` is currently unreachable.
